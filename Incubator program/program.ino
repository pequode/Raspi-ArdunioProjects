...
void changeTemp(double tempC,double tempR){
    double topTolerance = 1.0;
    if(tempC>TargetTemp){
      if(tempC>TargetTemp+topTolerance){//way too hot
          OpenOutside(true);
          OpenInside(true);
          turnOnFanWell(true);
          turnOnFanRes(true);
      ...
      if(tempC<TargetTemp-topTolerance){//way too cold
          OpenOutside(false);
          OpenInside(tempR>TargetTemp);
          turnOnFanWell(tempR>TargetTemp);
          turnOnFanRes(tempR>TargetTemp);
          turnOnHeat(true);
      }else{
          OpenOutside(false);
          OpenInside(true);
...
void loop()
{
  ...
  //double TempR = getCurrentTempRes();
  //double OutValue = PID(TempC);
  //double ResOutValue = PID(TempR);
  //changeTemp(OutValue,OutValue);
  //Disp(TempC);
  float c = getCurrentTempRes();
  c = (c *(9.0/5.0))+32;
  lcd.clear();
  lcd.setCursor(0,0);
  lcd.print(c);
